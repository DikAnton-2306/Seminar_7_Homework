...
void FindNumber(string meseg, int[,] arr)
{
    ...
    if (numberX <= arr.GetLength(0) && numberY <= arr.GetLength(1))
        System.Console.WriteLine("Там сидит число: " + arr[numberX - 1, numberY - 1]);
    else
        System.Console.WriteLine("Позиция вне матрицы");
}
...
PrintMatrix(matrix);
FindNumber("Укажите позицию элемента в матрице (через ;)", matrix);